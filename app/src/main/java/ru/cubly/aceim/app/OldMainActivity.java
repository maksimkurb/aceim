...
import ru.cubly.aceim.app.utils.LinqRules.AccountListPageLinqRule;
import ru.cubly.aceim.app.utils.LinqRules.AccountPageLinqRule;
import ru.cubly.aceim.app.utils.LinqRules.BuddyPageLinqRule;
import ru.cubly.aceim.app.utils.LinqRules.FileProgressPageLinqRule;
import ru.cubly.aceim.app.utils.LinqRules.MessagePageLinqRule;
import ru.cubly.aceim.app.utils.LinqRules.ProtocolAccountPageLinqRule;
import ru.cubly.aceim.app.utils.ViewUtils;
import ru.cubly.aceim.app.widgets.bottombar.BottomBarButtonInfo;

public class OldMainActivity extends AceIMActivity {
	...
	private void initCoreService() {
		if (mCoreServiceIntent == null) {
			mCoreServiceIntent = new Intent("ru.cubly.aceim.app.service.CoreService");
		}
				...
				public void run() {
					try {
						if (message instanceof FileMessage) {
							DialogUtils.showFileMessageDialog((FileMessage) message, mCoreService.getBuddy(message.getServiceId(), message.getContactUid()), OldMainActivity.this);
						} else if (message instanceof ServiceMessage) {
							ServiceMessage sm = (ServiceMessage) message;
							if (sm.isRequireAcceptDeclineAnswer()) {
								DialogUtils.showAcceptDeclineDialog(message, mCoreService.getBuddy(message.getServiceId(), message.getContactUid()), OldMainActivity.this);
							}
						} else {
			...
			runOnUiThread(new Runnable() {
				...
				public void run() {
					try {
						...
						Page.getInputFormPage(OldMainActivity.this, feature, info, p);
					...
					case MODIFIED:
						...
						if (account.isEnabled()) {
							if (pages.size() > 0) {
								...
								Page.getContactListPage(OldMainActivity.this, account);
							}
						} else {
							for (Page page : pages) {
	...
	public void openOptions(Account mAccount) {
		...
		getScreen().postDelayed(new Runnable() {
			...
			public void run() {
				...
				//Forcing OldMainActivity to die, as of its "singleTask" lifecycle,
